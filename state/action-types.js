export const COURSE_LOAD_REQUEST = 'COURSE_LOAD_REQUEST';
export const COURSE_LOAD_SUCCESS = 'COURSE_LOAD_SUCCESS';
export const COURSE_LOAD_FAILURE = 'COURSE_LOAD_FAILURE';
export const SENDBIRD_CHANNEL_FETCH_REQUEST = 'SENDBIRD_CHANNEL_FETCH_REQUEST';
export const SENDBIRD_CHANNEL_FETCH_SUCCESS = 'SENDBIRD_CHANNEL_FETCH_SUCCESS';
export const SENDBIRD_CHANNEL_FETCH_FAILURE = 'SENDBIRD_CHANNEL_FETCH_FAILURE';
export const SENDBIRD_CONNECT_REQUEST = 'SENDBIRD_CONNECT_REQUEST';
export const SENDBIRD_CONNECT_SUCCESS = 'SENDBIRD_CONNECT_SUCCESS';
export const SENDBIRD_CONNECT_FAILURE = 'SENDBIRD_CONNECT_FAILURE';
export const SENDBIRD_DISCONNECT_REQUEST = 'SENDBIRD_DISCONNECT_REQUEST';
export const SENDBIRD_DISCONNECT_SUCCESS = 'SENDBIRD_DISCONNECT_SUCCESS';
export const SENDBIRD_DISCONNECT_FAILURE = 'SENDBIRD_DISCONNECT_FAILURE';
export const SENDBIRD_MESSAGE_SEND_REQUEST = 'SENDBIRD_MESSAGE_SEND_REQUEST';
export const SENDBIRD_MESSAGE_SEND_SUCCESS = 'SENDBIRD_MESSAGE_SEND_SUCCESS';
export const SENDBIRD_MESSAGE_SEND_FAILURE = 'SENDBIRD_MESSAGE_SEND_FAILURE';
export const SENDBIRD_PROFILE_CHANGE_REQUEST = 'SENDBIRD_PROFILE_CHANGE_REQUEST';
export const SENDBIRD_PROFILE_CHANGE_SUCCESS = 'SENDBIRD_PROFILE_CHANGE_SUCCESS';
export const SENDBIRD_PROFILE_CHANGE_FAILURE = 'SENDBIRD_PROFILE_CHANGE_FAILURE';
export const USER_DATA_REQUEST = 'USER_DATA_REQUEST';
export const USER_DATA_SUCCESS = 'USER_DATA_SUCCESS';
export const USER_DATA_FAILURE = 'USER_DATA_FAILURE';
export const USER_DATA_UPDATE = 'USER_DATA_UPDATE';
export const USER_LOGIN_REQUEST = 'USER_LOGIN_REQUEST';
export const USER_LOGIN_SUCCESS = 'USER_LOGIN_SUCCESS';
export const USER_LOGIN_FAILURE = 'USER_LOGIN_FAILURE';
export const USER_TOKEN_CHANGE = 'USER_TOKEN_CHANGE';
